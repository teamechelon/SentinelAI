...
export default async function AttackLabPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="mx-auto max-w-[1300px] space-y-6">
    <section className="border-b border-border pb-5"><div className="tech-label">Sentinel / Controlled simulation</div><h1 className="mt-2 text-[22px] font-semibold tracking-[-0.025em]">Attack Lab</h1><p className="mt-1 max-w-3xl text-[12px] text-[var(--text-secondary)]">Execute deterministic multi-event sequences against a selected demo identity. Events use the existing detection path; sequence findings remain separate from event risk.</p></section>
