...
export default async function UsersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5">
    <section className="flex flex-col gap-3 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="tech-label">Sentinel / Identities</div><h1 className="mt-2 text-[22px] font-semibold tracking-[-0.025em]">Users</h1><p className="mt-1 text-[12px] text-[var(--text-secondary)]">Personal baselines, peer context, activity, and persisted alerts.</p></div><div aria-live="polite" className="font-mono text-[11px] text-[var(--text-muted)]"><span className="text-foreground">{result.page.total}</span> monitored users</div></section>
