...
export default async function ActivityPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const result = await getSentinelDataSource().listActivity({
    q: values.q, department: values.department, activityType: values.activityType, scenario: values.scenario,
  ...
  return <div className="mx-auto max-w-[1700px] space-y-5">
    <section className="flex flex-col gap-3 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="tech-label">Sentinel / Telemetry</div><h1 className="mt-2 text-[22px] font-semibold tracking-[-0.025em]">Activity</h1><p className="mt-1 text-[12px] text-[var(--text-secondary)]">Persisted events and their existing detection outcomes.</p></div><div aria-live="polite" className="font-mono text-[11px] text-[var(--text-muted)]"><span className="text-foreground">{result.page.total}</span> events · page {result.page.page} / {result.page.totalPages}</div></section>
